...
2. **安全去重** - 分层哈希（size → xxh3 → 完整 BLAKE3）+ 感知匹配，并排对比审阅，只删到回收站。*（开发中 - M4）*
3. **归整照片库** - 全部整理进你自己选择的库文件夹，目录结构可自定义（默认 `YYYY\YYYY-MM`），按拍摄日期命名、绝不冲突，附带完整撤销日志。*（开发中 - M5）*
...
- 🕐 **时区感知** - 首次运行时选择时区；日期过滤和（即将到来的）照片库文件名都遵循它。
...
| M4 | 精确去重：分层哈希、2-4 张并排对比 UI、键盘优先审阅 | ⏳ |
| M5 | 归整：自选库文件夹、可配置日期结构、原子移动、撤销日志 | ⏳ |
